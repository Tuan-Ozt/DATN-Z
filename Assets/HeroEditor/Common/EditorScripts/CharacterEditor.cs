...
namespace Assets.HeroEditor.Common.EditorScripts
{
    ...
    public class CharacterEditor : CharacterEditorBase
    {
        ...
        /// </summary>
        /// 
        //xử lý dữ liệu nhân vật khi chọn nhân vật , đồng bộ fusion khi ấn Play
        public NetworkRunner NetworkRunnerPrefab;
        public PlayerSpawner playerSpawner; // gán trong inspector hoặc tìm bằng code
        public async void StartFusionSession(string sceneName)
        {
            ...
            // Lưu lại scene trước khi chuyển (nếu cần quay lại sau)    
            ...
            else if (sceneName.StartsWith("Test"))
            {
                Map.ReturnSceneName = SceneManager.GetActiveScene().name;
            }

            // Ghi JSON nhân vật đã chọn
            string characterJson = Character.ToJson();
            PlayerDataHolder1.CharacterJson = characterJson;
            Debug.Log("Dữ liệu người chơi " + PlayerDataHolder1.CharacterJson);
            AuthManager.Instance.StartCoroutine(AuthManager.Instance.SaveCharacterToServer(PlayerDataHolder1.CharacterJson));
            Debug.Log("Đã lưu dữ liệu nhân vật trên database ");
            ...
            // --- Đăng ký callback PlayerSpawner ở đây ---
            ...
            // Lấy index build scene
            int index = GetBuildIndexFromSceneName(sceneName);
            if (index == -1) return;
